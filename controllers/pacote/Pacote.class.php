...
class Pacote extends Cupom
{
    ...
    private function getpacoteperuser(){
        ...
        $Usuario = new Exibir();
        $Usuario->exeExibir("SELECT id_usuario FROM usuario WHERE codigo = :codigo", NULL, NULL, "codigo={$this->ID}", FALSE);

        $Pacote = new Exibir();
        $Pacote->exeExibir("SELECT id_pacote FROM pacote_interacao WHERE id_usuario = :id", NULL, NULL, "id={$Usuario->Resultado()[0]["id_usuario"]}", FALSE);

        $this->Retorno = json_encode($Pacote->Resultado());
    }
